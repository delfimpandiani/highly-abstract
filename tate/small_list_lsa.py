...
def get_binary_values(hlac_id): 
    ...
    len_of_values_list = 826 #first check how long the output list should be
    ...
    for term in ex_list_of_terms:
        if term in bag_of_words:
            index = ex_list_of_terms.index(term)
            binary_values_list[index] = 1
            count += 1
    with open("output/hlacs_binary_values/smalllist/" + str(hlac_id) + "_binary_values.json", 'w') as file:
...
def create_matrix_list(a, b, c, d, e, f):
    ...
    print("length of this matrix should be 4956 and it is ", len(matrix_list))
    with open("output/hlacs_binary_values/smalllist/__attempt_at_matrix.json", 'w') as file:
...
def get_this_example_total_list():
    ...
    with open('output/ex_list_of_terms.json', 'w') as file:
             file.write(json.dumps(ex_list_of_terms))
    print("THE LENGTH OF THE NEW LIST IS", len(ex_list_of_terms))
